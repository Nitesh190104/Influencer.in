...
exports.unfollowUser = async (req, res) => {
    try {
        ...
        // Delete any pending follow request notification
...
exports.removeFollower = async (req, res) => {
    try {
        ...
        // Remove the follow where follower=followerId and following=currentUser
        const result = await Follow.findOneAndDelete({
            follower: followerId,
            following: userId
        });
    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                isMutual: !!(isFollowing && isFollowing.status === 'accepted' && isFollower && isFollower.status === 'accepted')
            }
        });
    } catch (error) {
...
exports.acceptFollowRequest = async (req, res) => {
    try {
        ...
        // Get the follow ID from notification data
        const followId = notification.data.followId;
        const follow = await Follow.findById(followId);

        if (!follow) {
            // Follow was deleted (user unfollowed), delete the notification
            await Notification.findByIdAndDelete(requestId);
            return res.status(404).json({
                success: false,
                message: 'Follow request no longer exists'
            });
        }
        ...
        // Create acceptance notification for the follower
        await Notification.create({
